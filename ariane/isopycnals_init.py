# make csv of each water parcel, its WM source, transport, and isopycnal that it lies on
...
def isopycnaldata(data_list):
    ...
    for datapath in data_list:
        ...
        salt = np.append(salt,data.init_salt[rebool].values)
        temp = np.append(temp,data.init_temp[rebool].values)
        depth = np.append(depth,data.init_depth[rebool].values)
        section = np.append(section,data.final_section[rebool].values)
        trans = np.append(trans,data.init_transp[rebool].values)
...
data.to_csv('/data1/bbeutel/LO_user/ariane/summary_files/isopycnals_init.csv')